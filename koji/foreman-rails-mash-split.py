...
class MashSplit(object):
    ...
    def mash_split(self, whole_path, tmp_path, split_path, mash_config, options=None, arches=None, compses=None, git_tag="HEAD", output_paths=None):
...
def main():
    ...
    s = MashSplit("/var/log/foreman-rails-mash-split.log")
    options = ["server"]
    arches = ["x86_64"]
    compses = ["comps-{}-rhel7.xml".format(scl)]
    whole_path = "/mnt/koji/releases/whole"
    tmp_path = "/mnt/koji/releases/tmp"
    split_path = "/mnt/koji/releases/split"

    mash_config = "foreman-rails-{}-rhel7".format(version)
    git_tag = scl
    output_paths = ["foreman-rails-{}/el7".format(version)]

    s.mash_split(whole_path, tmp_path, split_path, mash_config, options=options, arches=arches, compses=compses, git_tag=git_tag, output_paths=output_paths)
