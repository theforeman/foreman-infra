...
def main():
    ...
    s = MashSplit("/var/log/foreman-client-mash-split.log")
    options = ["server"]
    arches = ["x86_64"]
    ...
    for dist, code in dists.items():
        ...
        compses = ["comps-foreman-client-{}.xml".format(dist)]
        output_paths = ["foreman-client-{}/{}".format(version, code)]
        s.mash_split(whole_path, tmp_path, split_path, koji_tag, options=options, arches=arches, compses=compses, git_tag=git_tag, output_paths=output_paths)
