pipeline {
    ...
    stages {
        ...
        stage('Run Pipeline') {
            steps {
                duffy_ssh("cd forklift && ansible-playbook pipelines/pipeline_foreman_nightly.yml -e 'forklift_state=up'", 'duffy_box', './')
            }
        }
    }
    ...
    post {
        always {
            script {
                duffy_ssh('cd forklift && ansible-playbook playbooks/collect_debug.yml -l pipeline-foreman-nightly-centos7', 'duffy_box', './')
