pipeline {
    ...
    stages {
        stage("Setup Environment") {
            steps {
                dir('foreman-packaging') { git(url: 'https://github.com/theforeman/foreman-packaging/', branch: env.branch) }
                dir('obal') { git(url: 'https://github.com/theforeman/obal', branch: 'master') }
            }
        }
        ...
        stage("Build RPM") {
            ...
            steps {
                dir('foreman-packaging') {
                    sh("""
                        export PYTHONPATH=${env.WORKSPACE}/obal

                        git-annex init
                        ./setup_sources.sh \${project}

                        mkdir -p ${env.WORKSPACE}/foreman-packaging/rel-eng/build
                        args="-o ${env.WORKSPACE}/foreman-packaging/rel-eng/build/"
