    ...
    stages {
        stage('Mash Koji Repositories') {
            ...
            steps {
                ...
                sh "ssh -o 'BatchMode yes' root@koji.katello.org katello-mash-split-3.10.py"
                    ...
                    steps {
                        ...
                        withCredentials([string(credentialsId: 'centos-jenkins', variable: 'PASSWORD')]) {
                            runPlaybook(
                                ...
                                inventory: 'localhost',
                                extraVars: ["jenkins_job_name=foreman-katello-3.10-test", "jenkins_username=foreman", "jenkins_password=${env.PASSWORD}"],
                                options: ['-b']
                            )
                        }
                    }
                ...
                stage('Upgrade test') {
                    ...
                    steps {
                        ...
                        withCredentials([string(credentialsId: 'centos-jenkins', variable: 'PASSWORD')]) {
                            runPlaybook(
                                playbook: 'ci/centos.org/ansible/jenkins_job.yml',
                                extraVars: ["jenkins_job_name=foreman-katello-upgrade-3.10-test", "jenkins_username=foreman", "jenkins_password=${env.PASSWORD}"],
                                options: ['-b']
                            )
                        }
                    }
        ...
        stage('Push RPMs') {
            ...
            steps {
                ...
                sh "ssh -i /var/lib/workspace/workspace/deploy_katello_repos_key/deploy_katello_repos_key katelloproject@fedorapeople.org \"cd /project/katello/bin && sh rsync-repos-from-koji 3.10\""
