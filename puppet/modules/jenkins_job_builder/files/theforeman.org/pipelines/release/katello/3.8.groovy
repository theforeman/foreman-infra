    ...
    stages {
        stage('Mash Koji Repositories') {
            ...
            steps {
                ...
                mash("katello-mash-split-3.8.py")
                    ...
                    steps {
                        ...
                        withCredentials([string(credentialsId: 'centos-jenkins', variable: 'PASSWORD')]) {
                            runPlaybook(
                                ...
                                inventory: 'localhost',
                                extraVars: ["jenkins_job_name": "foreman-katello-3.8-test", "jenkins_username": "foreman"],
                ...
                stage('Upgrade test') {
                    ...
                    steps {
                        ...
                        withCredentials([string(credentialsId: 'centos-jenkins', variable: 'PASSWORD')]) {
                            runPlaybook(
                                playbook: 'ci/centos.org/ansible/jenkins_job.yml',
                                extraVars: ["jenkins_job_name": "foreman-katello-upgrade-3.8-test", "jenkins_username": "foreman"],
                                sensitiveExtraVars: ["jenkins_password": "${env.PASSWORD}"]
                            )
                        }
                    }
...
void repoclosure(repo, dist, additions = []) {
    ...
    node('el') {
        git url: "https://github.com/theforeman/foreman-packaging", branch: "rpm/1.19", poll: false
