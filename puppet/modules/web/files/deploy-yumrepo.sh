...
do_rsync() {
	...
	for d in "${REPO_INSTANCE_PATH}"/*; do
		(
			...
			latest=$(ls -t pulpcore-repos-[0-9]*.rpm 2>/dev/null | head -n1)
			if [[ -n "$latest" ]] ; then
				ln -sf "$latest" pulpcore-repos-latest.rpm
			fi
...
purgecdn() {
	...
	for d in "${REPO_PATH}"/*; do
		...
		fastly-purge ${purge_base} foreman-release.rpm foreman-client-release.rpm katello-repos-latest.rpm pulpcore-repos-latest.rpm
	done
	set -f
}
